#!/usr/bin/env python3
# Fork from https://github.com/josegonzalez/python-github-backup on MIT
...
def retrieve_repositories(args, authenticated_user):
    ...
    if args.all_starred:
        ...
        starred_repos = retrieve_data(
            args, starred_template, single_request=False)
...
def filter_repositories(args, unfiltered_repositories):
    ...
    if not args.private:
        repositories = [r for r in repositories if not r.get(
            'private') or r.get('public')]
    if languages:
    ...
    for repository in repositories:
        if repository.get('is_gist'):
            repo_cwd = os.path.join(
                output_directory, 'gists', repository['id'])
            ...
            repo_cwd = os.path.join(
                output_directory, 'starred', repository['owner']['login'], repository['name'])
        else:
            repo_cwd = os.path.join(
        ...
        if (args.include_repository or args.include_everything) \
            ...
            repo_name = repository.get('name') if not repository.get(
                'is_gist') else repository.get('id')
...
def fetch_repository(name,
    ...
    if clone_exists:
        ...
        if lfs_clone:
            git_command = ['git', 'lfs', 'fetch',
                           '--all', '--force', '--tags', '--prune']
        else:
        ...
        if bare_clone:
            if lfs_clone:
                git_command = ['git', 'lfs', 'clone',
                               '--mirror', remote_url, local_dir]
            else:
